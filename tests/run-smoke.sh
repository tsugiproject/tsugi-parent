...
if [ "$1" == "--watch" ]; then
    echo "Running Tsugi-Parent Smoke Tests in WATCH MODE..."
    echo "Browser window will be visible - you can watch the tests run!"
    echo "Make sure your server is running at http://localhost:8888/tsugi-parent/"
    echo ""
    php tests/SmokeTest.php --watch
else
    echo "Running Tsugi-Parent Smoke Tests..."
    echo "Make sure your server is running at http://localhost:8888/tsugi-parent/"
